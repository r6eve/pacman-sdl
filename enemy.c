  ...
  case ENEMY_GUZUTA:
    ...
    if (is_mving) {
      ...
      if (check_map_state(Enemy[index].block_x, Enemy[index].block_y) == 2) {
        ...
        int front_block_x = Enemy[index].block_x + front_pos[Enemy[index].dir][0];
        int front_block_y = Enemy[index].block_y + front_pos[Enemy[index].dir][1];
        int front_block = check_map_state(front_block_x, front_block_y);
        if ((front_block == 3) || (front_block == 4) || (front_block == 5) || (front_block == 6) || (front_block == 7) || (front_block == 8)) {
          front_block = 0;  /* can move it */
        }

        int left_block_x = Enemy[index].block_x + left_pos[Enemy[index].dir][0];
        int left_block_y = Enemy[index].block_y + left_pos[Enemy[index].dir][1];
        int left_block = check_map_state(left_block_x, left_block_y);
        if ((left_block == 3) || (left_block == 4) || (left_block == 5) || (front_block == 8)) {
          left_block = 0;
        }

        int right_block_x = Enemy[index].block_x + right_pos[Enemy[index].dir][0];
        int right_block_y = Enemy[index].block_y + right_pos[Enemy[index].dir][1];
        int right_block = check_map_state(right_block_x, right_block_y);
        if ((right_block == 3) || (right_block == 4) || (right_block == 5) || (front_block == 8)) {
          right_block = 0;
        }

        if (((rand()%100) == 0) ||
            ((390 <= Power_chara_mode[0])&&(Power_chara_mode[0] <= 400)) || ((390 <= Power_chara_mode[1])&&(Power_chara_mode[1] <= 400))) {
          if (check_map_state(Enemy[index].block_x + back_pos[Enemy[index].dir][0], Enemy[index].block_y + back_pos[Enemy[index].dir][1]) == 2) {
        ...
        } else if ((front_block == 0) && (left_block == 0) && (right_block == 0)) {
          int next = rand() % 3;
          if (next == 0) {
...
void mv_lose_enemy(int index) {
  ...
  if (is_mving) {
    ...
    if (now_value > Home_way[Enemy[index].block_y - 1][Enemy[index].block_x]) {
      --Enemy[index].nextblock_y;
    } else if (now_value > Home_way[Enemy[index].block_y][Enemy[index].block_x - 1]) {
      --Enemy[index].nextblock_x;
    } else if (now_value > Home_way[Enemy[index].block_y][Enemy[index].block_x + 1]) {
      ++Enemy[index].nextblock_x;
    } else if (now_value > Home_way[Enemy[index].block_y + 1][Enemy[index].block_x]) {
      ++Enemy[index].nextblock_y;
    }
  }
}
...
void check_hit_enemy() {
  ...
  for (int i = 0; i < NUM_ENEMY; ++i) {
    int d = get_distance(main_chara_pos_x, main_chara_pos_y, Enemy[i].pos_x, Enemy[i].pos_y);
    if (d < HIT_DISTANCE) {
  ...
  if (Game_mode == GAME_MODE_BATTLE) {
    ...
    for (int i = 0; i < NUM_ENEMY; ++i) {
      int d = get_distance(rival_chara_pos_x, rival_chara_pos_y, Enemy[i].pos_x, Enemy[i].pos_y);
      if (d < HIT_DISTANCE) {
